...
function clerkApiErrors(json) {
  const list = json && Array.isArray(json.errors) ? json.errors : [];
  return list.map((item) => ({
    code: item && item.code ? String(item.code) : "",
    message: item ? String(item.long_message || item.longMessage || item.message || "") : "",
  }));
}

function clerkErrorBlob(errors, fallback) {
  const parts = errors.map((e) => [e.code, e.message].filter(Boolean).join(": ")).filter(Boolean);
  return parts.join(" ") || String(fallback || "");
}

async function clerkBackendPost(pathname, body) {
...
function waitlistDisabled(blob) {
  return /waitlist.*(disabled|not enabled|unavailable)|feature_not_enabled|waitlist_not_enabled|not.?enabled.*waitlist/i.test(blob);
}

function alreadyWaitlisted(blob) {
  return /already.*(waitlist)|duplicate_record|waitlist_entry_exists/i.test(blob);
}

function alreadyClerkUser(blob) {
  return /already.*(user|member|account)|user_exists|email_address_taken|identifier_already_exists/i.test(blob);
}

/**
 * Public request-access. Prefers Clerk Waitlist (Marco invites from there).
 * If Waitlist is disabled, queues a Clerk invitation with notify:false so the
 * email appears under Users → Invitations without auto-emailing a join link.
 */
export async function requestClerkAccess(email) {
  const id = normalizeEmail(email);
  if (!id) {
  ...
  if (!clerkConfigured()) {
    return {
      ...
      message: "This server has no CLERK_SECRET_KEY, so the request could not be stored in Clerk.",
    };
  }

  try {
    const wait = await clerkBackendPost("/waitlist_entries", {
      email_address: id,
      notify: false,
    });
    if (wait.res.ok) {
      const status = wait.json && wait.json.status ? String(wait.json.status) : "pending";
      return { ok: true, via: "waitlist", status };
    }
    const errors = clerkApiErrors(wait.json);
    const blob = clerkErrorBlob(errors, wait.text);
    if (alreadyWaitlisted(blob)) {
      return { ok: true, via: "waitlist", status: "pending" };
    }
    if (alreadyClerkUser(blob)) {
      return {
        ok: false,
        error: "already_user",
        message: "That email already has a Clerk user. Use Sign in with Google.",
      };
    }
    if (wait.res.status === 401 || wait.res.status === 403) {
      return {
        ok: false,
        error: "clerk_auth",
        message: "Clerk rejected CLERK_SECRET_KEY. Check the secret on this Vercel environment matches the publishable key.",
      };
    }
    if (!waitlistDisabled(blob) && wait.res.status !== 404) {
      return {
        ok: false,
        error: "clerk_waitlist",
        message: blob || "Clerk did not accept this email for the waitlist.",
      };
    }

    const invite = await clerkBackendPost("/invitations", {
      email_address: id,
      ignore_existing: true,
      notify: false,
    });
    if (invite.res.ok) {
      return { ok: true, via: "invitation", status: "queued" };
    }
    const inviteBlob = clerkErrorBlob(clerkApiErrors(invite.json), invite.text);
    if (alreadyClerkUser(inviteBlob) || alreadyWaitlisted(inviteBlob) || /already_invited|invitation_already/i.test(inviteBlob)) {
      return {
        ok: true,
        via: "invitation",
        status: "queued",
    ...
    return {
      ok: false,
      error: "clerk_waitlist",
      message: waitlistDisabled(blob)
        ? "Clerk Waitlist is off, and an invitation could not be queued. Enable Waitlist (Configure → Waitlist) or invite the email from Users → Invitations."
        : (inviteBlob || blob || "Clerk did not store this request."),
    };
  } catch (e) {
    return {
      ok: false,
      error: "clerk_unreachable",
      message: e && e.message ? String(e.message) : "Could not reach Clerk.",
    };
  }
}
